  ...
  return (
    <aside className="bg-[var(--color-navigation-section)] backdrop-blur-xl p-4 rounded-sm fixed w-65 z-50 h-full animate-pulse">
      <div>
        ...
        <div
          className={`h-4 w-24 ${theme === 'light' ? 'bg-gray-500' : 'bg-pink-900'} rounded mb-2`}
        />

        {/* Image Placeholder */}
        <div
          className={`w-full h-60 ${theme === 'light' ? 'bg-gray-600' : 'bg-pink-950'} rounded-md`}
        />

        {/* Station Name + Icon Placeholder */}
        <div className="flex items-baseline gap-2 mt-2">
          <div
            className={`h-6 w-28 ${theme === 'light' ? 'bg-gray-400' : 'bg-pink-800'} rounded`}
          />
          <div className={`h-6 w-6 ${theme === 'light' ? '' : 'bg-pink-900'} rounded-full`} />
        </div>
        ...
        <div className="stationDetails mt-2 space-y-3 text-[var(--color-text-secondary)]">
          {[...Array(4)].map((_, i) => (
            <div key={i} className="flex items-center gap-2">
              <div
                className={`h-4 w-4 ${theme === 'light' ? 'bg-gray-500' : 'bg-pink-900'} rounded`}
              />
              <div
                className={`h-3 w-24 ${theme === 'light' ? 'bg-gray-400' : 'bg-pink-800'} rounded`}
              />
            </div>
          ))}
        </div>
